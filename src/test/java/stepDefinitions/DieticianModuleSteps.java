...
import utilities.Tokens;
	...
	public class DieticianModuleSteps {
		...
		public static String deiticianLoginId;
		public static String deiticianLoginPwd;
...
;
		...
		public void admin_generates_dietician_token_and_saves_Dietician_Token() {
			...
			deiticianLoginId = Tokens.dietIdMap.get(singleDataRow.get("DieticianLogin"));
			deiticianLoginPwd = Tokens.dietIdMap.get(singleDataRow.get("DieticianPwd"));
			...
			APIFunction deitlogin= new APIFunction(null,APIConstant.USER_LOGIN_ENDPOINT, APIConstant.POST, 
					new Gson().toJson(deiticianLogin));
			ResponseOptions<Response> resp= deitlogin.ExecuteAPI();
		
			//Saving Token

			DieticianToken=Tokens.dietIdMap.put("dietBearerToken",resp.body().path("token"));
		System.out.println("Saving token; Key :  DieticianToken , Value : "+resp.body().path("token"));
		
//		if (resp.getStatusCode()==201) {
//			System.out.println(singleDataRow.get("Dieticiantoken")+" "+ resp.body().path("token"));
//			Tokens.dietIdMap.put(singleDataRow.get("Dieticiantoken"),resp.body().path("token"));
//		}
		    
		}
//		@Given("Admin creates dietician Post request without Auth and with request body for {string}")
//		public void admin_creates_dietician_post_request_without_auth_and_with_request_body_for(String testcase) {
//			String dieticianRequestBody=DataHandler.GetDieticianRequestBodyForInvalid(testcase);
//			
//					System.out.println("dietician Request Body---->"+dieticianRequestBody);
//			
//					apiFunction=new APIFunction(dieticianRequestBody,APIConstant.CREATE_DIETICIAN_ENDPOINT,
//							   APIConstant.POST,singleDataRow);
//		}
		
		@Given("Admin creates dietician Post request without Auth and with request body for {string} and {string}")
		public void admin_creates_dietician_post_request_without_auth_and_with_request_body_for_and(String sheet, String row) {
			 singleDataRow=DataHandler.getDataRow(sheet,row);
		       String dietReqBody = DataHandler.GetDieticianRequestBody(sheet);
			   apiFunction=new APIFunction(dietReqBody,APIConstant.CREATE_DIETICIAN_ENDPOINT,
					   APIConstant.POST,singleDataRow,null );
		}
		
		@Given("Admin creates dietician Post request with DieticianToken and with request body for {string} and {string}")
		public void admin_creates_dietician_post_request_without_dietician_token_and_with_request_body_for_and(String sheet, String row) {
			 singleDataRow=DataHandler.getDataRow(sheet,row);
		       String dietReqBody = DataHandler.GetDieticianRequestBodyExistingDietician(sheet);
			   apiFunction=new APIFunction(dietReqBody,APIConstant.CREATE_DIETICIAN_ENDPOINT,
					   APIConstant.POST,singleDataRow,Tokens.dietIdMap.get("Dieticiantoken") );
		}		
